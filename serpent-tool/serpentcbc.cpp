...
size_t serpent_cbc_encrypt(const uint8_t* plaintext, size_t plain_len,const uint32_t* round_keys, const uint8_t* iv,
                           uint8_t* ciphertext, size_t cipher_max) {
    size_t pad_len = 16 - (plain_len % 16);
    size_t padded_len = plain_len + pad_len;

    if (padded_len > cipher_max) {
    ...
    for (size_t i = 0; i < pad_len; ++i) {
        ciphertext[plain_len + i] = (uint8_t)pad_len;
    }
...
size_t serpent_cbc_decrypt(const uint8_t* ciphertext, size_t cipher_len, const uint32_t* round_keys, const uint8_t* iv,
    ...
    for (uint8_t i = 1; i <= pad_len; ++i) {
        if (plaintext[cipher_len - i] != pad_len) {
            throw runtime_error("Битый паддинг");
        }
    }
